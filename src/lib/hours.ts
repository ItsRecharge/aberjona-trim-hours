...
// here take an optional `goal` and fall back to DEFAULT_YEARLY_HOURS_GOAL so
...
export const DEFAULT_YEARLY_HOURS_GOAL = 10.0;
/** @deprecated Use the configurable goal from ChapterSettings where possible. */
export const YEARLY_HOURS_GOAL = DEFAULT_YEARLY_HOURS_GOAL;
...
export function hoursRemaining(
  earned: number,
  goal: number = DEFAULT_YEARLY_HOURS_GOAL,
): number {
