      ...
      {step === 2 && (
        <div className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Name your chapter and set the yearly service-hours goal.
            </p>
          </div>
          <div>
            ...
            <input
              ...
              placeholder="Aberjona Chapter"
            />
          </div>
          <div>
            <label className={labelClass}>Yearly hours goal</label>
            <input
              type="number"
              min={1}
              className={fieldClass}
              value={form.yearlyHoursGoal}
              onChange={set("yearlyHoursGoal")}
            />
          </div>
        </div>
      )}
      ...
      {step === 4 && (
        <div className="space-y-4">
          ...
          <dl className="divide-y divide-gray-100 rounded-lg border border-gray-200 text-sm">
            ...
            <Row label="Yearly goal" value={`${form.yearlyHoursGoal} hours`} />
            <Row
              label="Email sending"
          ...
          <form action={formAction}>
            ...
            <input type="hidden" name="yearlyHoursGoal" value={form.yearlyHoursGoal} />
...
function validateStep(step: number, form: FormData): string | null {
  ...
  if (step === 2) {
    ...
    const goal = Number(form.yearlyHoursGoal);
    if (!Number.isFinite(goal) || goal <= 0)
      return "Yearly hours goal must be a positive number.";
  }
  if (step === 3) {
